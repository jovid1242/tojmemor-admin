...
export default function Index() {
    ...
    const addSlider = (title, text, url, image) => {
        setPost((prevState) => [...prevState, { title, text, url, image }])
    }
    ...
    const deleteSlider = (id) => {
        ...
        // post.splice(res, 1)
        setPost((prev) => prev.splice(res, 1))
    }
    ...
    useEffect(() => {
        http.get(`get_slider?page=${pagePost}`).then((response) => {
            setPost(response.data.slider.rows)
