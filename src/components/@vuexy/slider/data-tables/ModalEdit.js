...
export default function ModalEdit({ show, closeModalEdit, editSlider, news }) {
    ...
    const submitForm = (e) => {
        ...
        data.append('position', post.position);
        http.put(`/update_slider/${news.id}`, data)
            .then((response) => {
                toggleModal()
                editSlider(post, preloadImg.image)
                notifySuccess('Слайдер успешно изменен! ')
            })
