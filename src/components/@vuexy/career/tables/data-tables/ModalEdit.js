...
export default function ModalEdit({ show, closeModalEdit, editCareer, career }) {
    ...
    useEffect(() => {
        ...
        setpreloadImg({image: career.img})
    }, [career])
    ...
    const submitForm = (e) => {
        ...
        // console.log(post);
        http.put(`/update_career/${post.id}`, data)
            .then((response) => {

                editCareer(post, preloadImg.image)
                toggleModal()
                notifySuccess('Карьера успешно изменено!')
            })
