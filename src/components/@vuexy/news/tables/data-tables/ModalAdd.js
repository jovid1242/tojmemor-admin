...
export default function ModalAdd({ show, closeModalAdd, addNews }) {
    ...
    const onEditorStateChange = editorState => {
        ...
        setPost({
            text: draftToHtml(convertToRaw(edit.editorState.getCurrentContent())),
            title: title,
            file: imageFile.file
        })
    }
    ...
    const submitForm = (e) => {
        ...
        console.log(post);
        // addNews(post)
        http.post('/create_news', data)
            .then((response) => {
                addNews(post.title, post.text, preloadImg.image)
                toggleModal()
