...
console.log('ss', post.length);
  ...
  return (
    <>
      <Card>
        <ModalWarning show={modal} closeModal={closeModal} id={idNews} deleteNews={deleteNews} />
        <ModalAdd show={modalAdd} addNews={addNews} closeModalAdd={closeModalAdd} />
        <ModalEdit show={modalEdit} editNews={editNews} closeModalEdit={closeModalEdit} news={news} />
        <CardHeader>
                ...
                post?.filter((val) => {
                  ...
                  return (
                    ...
                    <th scope="row">{((pagePost-1) * 6) + (index + 1)}</th>
                      ...
                      <td>{el.title}</td>
                      <td>{ReactHtmlParser(el.text.substr(0, 65) + '...')}</td>
                      <td>
