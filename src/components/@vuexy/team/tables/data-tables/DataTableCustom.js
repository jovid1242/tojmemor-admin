...
export default function Index() {
  ...
  useEffect(() => {
    http.get(`get_team?page/${pagePost}`)
      .then((response) => {
        setPost(response.data.team)
  ...
  return (
    <>
      <Card>
        <ModalWarning show={modal} closeModal={closeModal} id={idNews} deleteTeam={deleteTeam} />
        <ModalAdd show={modalAdd} addTeam={addTeam} closeModalAdd={closeModalAdd} />
        <ModalEdit show={modalEdit} editTeam={editTeam} closeModalEdit={closeModalEdit} team={team} />
        <CardHeader>
            ...
            <tbody>
              {
                post?.filter((element) => {
                  if (!searchPost === null) {
                    ...
                    element.name?.toLowerCase().includes(searchPost.toLowerCase())
                  ) {
                    return element
                  }
                }).map((el, index) => {
                      ...
                      <td>
                        <div className="d-flex">
                          <Icon.Edit
                            ...
                            outline
                            id="positionTop"
                            onClick={() => { setModalEdit(true); setTeam(el) }}
                          />
